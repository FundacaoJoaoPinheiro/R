...
library(janitor)  #usado para manipular tibbles.
...
local_planilha_final <- "resultado/resultado_script.xlsx"  #nome da planilha a ser feita
...
calcular <- function(x) {  #função para calcular receita (x) específica
  tce_m %>% 
    ...
    filter(str_detect(descricao_da_receita, fixed(x, ignore_case = TRUE)) == TRUE) %>%
    summarise(!!x := sum(receita_liquida))
}   #agrupa por municipio e filtra se receita = x. !! = programação dplyr.


lista_calculados <- map(receitas$nome_base_tce, calcular)
...
write.xlsx(
  ...
  sheetName = str_c("receitas por municipio", as.character(ano)),
  colNames = TRUE,
  rowNames = FALSE,
  colWidths = "auto",
  overwrite = TRUE
