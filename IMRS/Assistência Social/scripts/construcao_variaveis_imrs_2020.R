...
## Populacao 2019
pop <- read_xls('data/estimativa_dou_2020.xls', sheet = 2) %>%
  filter(`COD. UF` == 31) %>%
  mutate(IBGE7 = paste(`COD. UF`, `COD. MUNIC`, sep = ""),
         IBGE6 = str_sub(IBGE7, 1, 6),
         ANO = 2020,
         `População total` = `POPULAÇÃO ESTIMADA`) %>%
  select(IBGE6, IBGE7, ANO, `População total`)
...
fam_cad <- read_csv("data/2020/cad e bpc/fam_cad_10_2020.csv") %>%
  filter(UF == "MG")

fam_cad_pbf <- read_csv("data/2020/cad e bpc/fam_pbf_10_2020.csv") %>%
  filter(UF == "MG")

fam_cad_mais_meio_sm <- read_csv("data/2020/cad e bpc/fam_cad_mais_meio_sm_10_2020.csv") %>%
  filter(UF == "MG")
...
#> BPC
bpc_deficiencia <- read_csv("data/2020/cad e bpc/bpc_deficiencia_12_2020.csv") %>%
  filter(UF == "MG")

bpc_idoso <- read_csv("data/2020/cad e bpc/bpc_idoso_12_2020.csv") %>%
  filter(UF == "MG")

bpc <- left_join(bpc_idoso, bpc_deficiencia[,c(1,5)], by = "Código") %>%
  left_join(pop[,c(1,4, 30:33)], by = c("Código" = "IBGE6")) %>%
  group_by(`Código`) %>%
